...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet) = {
    /*GoogleVsApple.counter = GoogleVsApple.counter + 1;println(GoogleVsApple.counter); /*println("start");that foreach println;*/ */
  ...
  private def max(singleTweet: Tweet, theSet: TweetSet): Tweet = {
    try {
      val mostFromTheSet = theSet.mostRetweeted
      if (singleTweet.retweets <= mostFromTheSet.retweets) mostFromTheSet
      else singleTweet
    } catch {
