...
object TweetReader {
  ...
  private def unionOfAllTweetSets(curSets: List[TweetSet], acc: TweetSet): TweetSet =
    if (curSets.isEmpty) {println(curSets.length);acc}
    else {acc foreach println;println(curSets.length);println("start");curSets.head foreach println;unionOfAllTweetSets(curSets.tail, acc.union(curSets.head))}
